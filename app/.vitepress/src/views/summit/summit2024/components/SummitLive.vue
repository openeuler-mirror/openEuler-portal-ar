...
function createUserId(liveId: number) {
  ...
  liveUrl.value = `https://hw.vhallyun.com/v2/watch/${liveId}?lang=ar&landScape=true`;
}
...
watch(
  ...
  () => {
    liveRoom.value =
      lang.value === 'ar' ? renderData[0].nameAr : renderData[0].nameEn;
  },
  {
    immediate: true,
  }
  ...
  <div class="live-room">
    ...
    <div class="live-room-web">
      <div class="live-room-web-itembox" :class="className">
        ...
        >
          <p class="name">{{ lang === 'ar' ? item.nameAr : item.nameEn }}</p>
        </div>
      </div>
    </div>
  </div>
